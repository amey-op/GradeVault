const ConfirmModal = ({ open, title, message, customBody, onConfirm, onCancel, confirmText = "Confirm", cancelText = "Cancel", isAlert = false }) => {
    if (!open) return null;
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="glass-panel w-full max-w-sm rounded-xl p-6 border border-gray-700 shadow-2xl animate-fade-in">
                ...
                {customBody ? customBody : <p className="text-gray-400 text-sm mb-6 whitespace-pre-wrap">{message}</p>}
